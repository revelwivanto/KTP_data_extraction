...
class Cropper:
    ...
    def run(self, np_image, output_dir='data/output'):
        """
        Read the signature extracted by Extractor, crop it, and save the cropped images to a directory.

        Params:
        - np_image: numpy array
          The input image from which signatures will be cropped.
        - output_dir: str
          Directory where cropped images will be saved. Default is 'data/output'.
        """
        ...
        # Save cropped regions as images in the output directory
        for key, value in regions.items():
            [x, y, w, h] = value
            cropped_image = np_image[y:y+h, x:x+w]  # Crop using numpy slicing

            # Create PIL Image object from cropped numpy array
            cropped_image_pil = Image.fromarray(cropped_image)

            # Save image to output directory
            image_filename = os.path.join(output_dir, f"cropped_region_{key}.png")
            cropped_image_pil.save(image_filename)
